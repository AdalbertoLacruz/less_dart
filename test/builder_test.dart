...
Future<Null> main() async {
  group('EntryPoints', () {
    test('default', () {
      final entryPoints = EntryPoints(<String>['*.less', '*.html']);
      expect(entryPoints.check(r'/web/test.less'), isTrue);
    });

    test('exclusion', () {
      final entryPoints = EntryPoints(<String>[
        'web/test.less',
        'web/dir1/*/dir4/*.html',
        '!/dir3/dir4/*.html'
      ])
    ...
    test('HTML builder', () async {
      final htmlProcess = DotHtmlBuilder()
        ..flags = <String>['--no-color', '-']
        ..inputContent = load('test/transformer/index_source.html');
      await htmlProcess.transform(customOptions);
      expect(htmlProcess.outputContent,
          equals(load('test/transformer/index_result.html')));
    });

    test('LESS builder', () async {
      final lessProcess = DotLessBuilder()
