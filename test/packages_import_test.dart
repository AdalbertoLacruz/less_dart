  ...
  group('packages_import', () {
    ...
    test('properly converts packages/ path in import', () async {
      ...
      final String packagesFileContent =
          'test_package:${uriParentFolder.toString()}';
      final String lessRelativePath = 'test/less/import-packages/test.less';
      final String lessAbsolutePath =
          uriParentFolder.resolve(lessRelativePath).toFilePath();

      final PackageResolver _resolver = await PackageResolver.loadConfig(
          Uri.dataFromString(packagesFileContent));
      final PackageResolverProviderMock _packageResolverProviderMock =
      ...
      final FileFileManager _fileManager =
          FileFileManager(Environment(), _packageResolverProviderMock);
      final FileLoaded result = await _fileManager.loadFile(
